...
namespace Evento.Infrastructure.Repositorires
{
    public class EventRepository : IEventRepository
    {
        private static readonly ISet<Event> _events = new HashSet<Event>()
        {
            ...
            new Event(Guid.NewGuid(), "Event 2", "Event 2 description", DateTime.UtcNow.AddHours(4), DateTime.UtcNow.AddHours(8))
        };
        ...
        public async Task<IEnumerable<Event>> BrowseAsync(string name = "")
        {
            ...
            {
                events.Where(x => x.Name.ToLowerInvariant().Contains(name.ToLowerInvariant()));
            }
